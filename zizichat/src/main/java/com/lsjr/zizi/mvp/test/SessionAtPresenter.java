package com.lsjr.zizi.mvp.test;
...
public class SessionAtPresenter extends BasePresenter<ISessionAtView> {
    ...
    public SessionAtPresenter(ISessionAtView mvpView, String sessionId, Conversation.ConversationType conversationType) {
        super(mvpView);
        for (int i = 0; i < 2; i++) {
    ...
    public void setAdapter() {
        if (mAdapter == null) {
            mAdapter = new SessionAdapter(UIUtils.getContext(), mData, this);
...
//                    mContext.jumpToActivity(intent);
                        ...
                        helper.getView(R.id.bivPic).setOnClickListener(v -> {
                            ...
                            if (isSend) {
                                if (fileMessage.getLocalPath() != null && new File(fileMessage.getLocalPath().getPath()).exists()) {
                                    FileOpenUtils.openFile(UIUtils.getContext(), fileMessage.getLocalPath().getPath());
                                } else {
                   ...
                   //RedPacketUtil.openRedPacket(((ChatActivity) mContext), chatType, redPacketId, redPacketType, receiverId, direct);
                }
            });
            getView().getRvMsg().setAdapter(mAdapter);
            mAdapter.setOnItemLongClickListener((helper, viewGroup, view, position) -> {
                View sessionMenuView = View.inflate(UIUtils.getContext(), R.layout.dialog_session_menu, null);
