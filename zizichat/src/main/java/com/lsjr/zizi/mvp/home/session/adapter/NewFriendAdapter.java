...
public class NewFriendAdapter extends ABaseRefreshAdapter<NewFriendMessage> {

	public NewFriendAdapter(Context context, List<NewFriendMessage> datas, int itemLayoutId) {
		super(context, datas, itemLayoutId);
	}

	@Override
	protected void convert(BaseRecyclerHolder viewHolder, NewFriendMessage friend, int position) {
		ImageView avatar_img = viewHolder.getView(R.id.avatar_img);
		TextView nick_name_tv = viewHolder.getView( R.id.nick_name_tv);
		TextView des_tv = viewHolder.getView( R.id.des_tv);
		Button action_btn_1 = viewHolder.getView(R.id.action_btn_1);
		Button action_btn_2 = viewHolder.getView( R.id.action_btn_2);
		ImageView right_img = viewHolder.getView( R.id.right_img);
		...
		friend1.set_id(friend.get_id());
		AvatarHelper.getInstance().displayAvatar(friend1, avatar_img, true);
		// 昵称
	...
	public void setmListener(NewFriendActionListener mListener) {
